...
class Orbit:
    ...
    def __init__(self,t0, zp: float, za: float, i: float, raan: float, theta_0 = 0, aop = 0):
        ...
        self.__RAAN_dot = -(3/2*(mu**0.5)*J2*(R**2)/(((1-e**2)**2)*(a**(7/2))))*np.cos(self.__i0)
        self.__w_dot = -(3/2*(mu**0.5)*J2*(R**2)/(((1-e**2)**2)*(a**(7/2))))*(2.5*(np.sin(self.__i0))**2-2)
        self.__T0 = 2*pi*a**1.5/np.sqrt(mu)
        self.__h0 = np.sqrt((R+zp)*mu*(1+self.__e0))

        #Eccentric anomaly
        E = np.arctan(np.sqrt((1-self.__e0)/(1+self.__e0))*np.tan(self.__theta0/2))
        #Mean anomaly:
